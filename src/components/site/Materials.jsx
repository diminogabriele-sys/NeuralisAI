    ...
    <section id="materiali" className="relative py-24 md:py-32 border-t border-steel">
      <div className="mx-auto max-w-[1600px] px-6 md:px-12">
        <div className="grid grid-cols-12 gap-6 mb-16">
          <div className="col-span-12 md:col-span-5">
            <div className="font-mono text-xs uppercase tracking-[0.3em] text-brabus mb-4">
              // Materiali & Artigianalità
            </div>
            <h2 className="font-display uppercase text-4xl md:text-7xl text-titanium leading-[0.95] break-words">
              Solo ciò che merita<br />di essere su una moto.
            </h2>
          </div>
        ...
        <div className="grid grid-cols-12 gap-px">
          <div className="col-span-12 lg:col-span-4 flex flex-col border border-steel">
            {materials.map((m) => (
              <button
                ...
                className={`text-left px-6 py-5 border-b border-steel last:border-b-0 transition-colors ${
                  active === m.id ? "bg-brabus/10" : "hover:bg-secondary/30"
                }`}
              >
                <div className={`font-display uppercase text-2xl ${active === m.id ? "text-brabus" : "text-titanium"}`}>
                  {m.name}
                </div>
          ...
          <div className="col-span-12 lg:col-span-8 border border-steel border-l-0 lg:border-l-0 p-8 md:p-10 bg-card/30 flex flex-col justify-between">
            <motion.div key={current.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
              <p className="text-lg text-titanium leading-relaxed max-w-xl mb-8">{current.desc}</p>
              <div className="grid grid-cols-2 gap-6 max-w-sm">
                {current.props.map((p) => (
                  <div key={p.l} className="border-l border-brabus/50 pl-4">
                    <div className="font-display uppercase text-3xl text-brabus leading-none">{p.v}</div>
                    <div className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground mt-2">{p.l}</div>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px mt-12 border border-steel">
          {stats.map((s) => (
            <div key={s.l} className="p-6 md:p-8 border-r border-steel last:border-r-0 text-center">
              <div className="font-display uppercase text-3xl md:text-4xl text-titanium">{s.v}</div>
              <div className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground mt-2">{s.l}</div>
            </div>
          ))}
        </div>
      </div>
